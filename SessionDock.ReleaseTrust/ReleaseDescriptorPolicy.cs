...
public static class ReleaseDescriptorPolicy
{
    ...
    public const string Product = "SessionDock";
        ...
        DateTimeOffset? now = null)
    {
        ...
        {
            throw new ReleaseTrustException(
                "The trusted SessionDock release key could not verify this update.",
                exception);
        }
        ...
        DateTimeOffset now)
    {
        if (descriptor.SchemaVersion != SchemaVersion ||
            (!IsCurrentIdentity(descriptor) && !IsLegacyIdentity(descriptor)))
        {
            throw new ReleaseTrustException(
                "The release descriptor is not for this SessionDock update channel.");
        }
    ...
    public static bool IsCurrentIdentity(ReleaseDescriptor descriptor)
    {
        ArgumentNullException.ThrowIfNull(descriptor);
        return descriptor.Product == Product &&
               descriptor.Repository == Repository &&
               descriptor.Channel == Channel &&
               descriptor.KeyId == KeyId;
    }

    public static bool IsLegacyIdentity(ReleaseDescriptor descriptor)
    {
        ArgumentNullException.ThrowIfNull(descriptor);
        return descriptor.Product == LegacyProduct &&
               descriptor.Repository == LegacyRepository &&
               descriptor.Channel == LegacyChannel &&
               descriptor.KeyId == LegacyKeyId;
    }
